#include "dualmode_rename.h"   // must precede UITask.h (renames classes under -D DUALMODE)
#include "UITask.h"
#include "target.h"
#include <Arduino.h>
...
#ifdef WITH_MT_BEACON
    // live Meshtastic beacon status (warning color when off)
    if (_beacon) {
      char b[40];
...
#endif
#ifdef WITH_CAR_NODE
    // live car-node beacon status (warning color when off)
    if (_carnode) {
      char b[40];
      _carnode->uiLine(b, sizeof(b));
      _display->setCursor(0, 40);
      _display->setColor(_carnode->enabled() ? UIColor::primary_txt : UIColor::warning_txt);
      _display->print(b);
    }
#endif
  }
